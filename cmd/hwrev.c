...
int do_hwrev(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (check_range(146, 186, hwrev_adc)) {
		env_set("devnum", "1");
		run_command("setenv -f PanChoEnvSize 0x1000; setenv -f PanChoEnvLoc 28560 /*seems clear on uboot.bin mbr and gpt*/", 0);
		...
		run_command("if env exists hwrev; then; echo \"skip PanChoEnv Load\"; else; echo \"start PanChoEnv Load\"; echo \"rescan mmc...\"; mmc rescan; mmc dev 1; echo \"list mmc...\"; mmc list; echo \"info mmc...\"; mmc info; ; echo \"###### before load ######\" ; printenv PanChoEnvLoaded; setenv -f PanChoEnvLoaded false; run LoadPanChoEnv; echo \"###### after load ######\"; printenv PanChoEnvLoaded; fi", 0);
		...
		env_set("devnum", "0");
		run_command("if env exists hwrev; then; env default -f -a ; fi", 0);
